...
def compute_mnre(pl_module, batch):
    infer = pl_module.infer(batch)
    mnre_logits = pl_module.mnre_classifier(infer["cls_feats"])

    mnre_labels = batch["labels"]
    mnre_labels = torch.tensor(mnre_labels).to(pl_module.device).long()
    mnre_loss = F.cross_entropy(mnre_logits, mnre_labels.view(-1))

    ret = {
        "mnre_loss": mnre_loss,
        "mnre_logits": mnre_logits,
        "mnre_labels": mnre_labels,
    }

    phase = "train" if pl_module.training else "val"
    if phase == "train":
        loss = getattr(pl_module, f"{phase}_mnre_loss")(ret["mnre_loss"])
        acc = getattr(pl_module, f"{phase}_mnre_accuracy")(
            ret["mnre_logits"], ret["mnre_labels"]
        )
        pl_module.log(f"mnre/{phase}/loss", loss)
        pl_module.log(f"mnre/{phase}/accuracy", acc)
    else:
        loss = getattr(pl_module, f"{phase}_mnre_loss")(ret["mnre_loss"])
        acc = getattr(pl_module, f"{phase}_mnre_accuracy")(
            ret["mnre_logits"], ret["mnre_labels"]
        )
        precision = getattr(pl_module, f"{phase}_mnre_precision")(
            ret["mnre_logits"], ret["mnre_labels"]
        )
        recall = getattr(pl_module, f"{phase}_mnre_recall")(
            ret["mnre_logits"], ret["mnre_labels"]
        )
        f1 = getattr(pl_module, f"{phase}_mnre_f1")(
            ret["mnre_logits"], ret["mnre_labels"]
        )
        getattr(pl_module, f"{phase}_mnre_class_accuracy")(
            ret["mnre_logits"], ret["mnre_labels"]
        )
...
def vqa_test_wrapup(outs, model_name):
    ...
    with open(f"vqa_submit.json", "w") as fp:
...
def arc_test_wrapup(outs, caplen, model_name):
    ...
    with open(f"coco_cap_len{caplen}.json", "w") as fp:
        json.dump(rets, fp, indent=4)
    ...
    os.remove(f"coco_cap_len{caplen}.json")
