        ...
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                if split == "train":
                    ...
                    setattr(pl_module, f"{split}_{k}_accuracy", Accuracy())
                    setattr(pl_module, f"{split}_{k}_loss", Scalar())
                    setattr(pl_module, f"{split}_{k}_precision", Precision(ignore_index=0))
                    setattr(pl_module, f"{split}_{k}_recall", Recall(ignore_index=0))
                    setattr(pl_module, f"{split}_{k}_f1", F1(ignore_index=0))
                    setattr(pl_module, f"{split}_{k}_class_accuracy", torchmetrics.Accuracy(num_classes=23, average='none'))
            else:
                setattr(pl_module, f"{split}_{k}_accuracy", Accuracy())
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            if phase == "train":
                ...
                pl_module.log(
                    f"other_metrics/{phase}/precision_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_precision").compute(),
                )
                getattr(pl_module, f"{phase}_{loss_name}_precision").reset()
                pl_module.log(
                    f"other_metrics/{phase}/recall_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_recall").compute(),
                )
                getattr(pl_module, f"{phase}_{loss_name}_recall").reset()
                pl_module.log(
                    f"other_metrics/{phase}/f1_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_f1").compute(),
                )
                getattr(pl_module, f"{phase}_{loss_name}_f1").reset()
                pl_module.log(
                    f"other_metrics/{phase}/peer_acc_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_class_accuracy").compute()[5],
                )
                pl_module.log(
                    f"other_metrics/{phase}/member_of_acc_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_class_accuracy").compute()[9],
                )
                pl_module.log(
                    f"other_metrics/{phase}/contain_acc_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_class_accuracy").compute()[15],
                )
                pl_module.log(
                    f"other_metrics/{phase}/present_in_acc_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_class_accuracy").compute()[16],
                )
                pl_module.log(
                    f"other_metrics/{phase}/locate_at_acc_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_class_accuracy").compute()[14],
                )
                pl_module.log(
                    f"other_metrics/{phase}/place_of_residence_acc_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_class_accuracy").compute()[10],
                )
                getattr(pl_module, f"{phase}_{loss_name}_class_accuracy").reset()
        else:
            value = getattr(pl_module, f"{phase}_{loss_name}_accuracy").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value)
            getattr(pl_module, f"{phase}_{loss_name}_accuracy").reset()
...
def set_schedule(pl_module):
    ...
    head_names = ["vqa_classifier", "nlvr2_classifier", "mlm_score", "itm_score", "snli_classifier", "mnre_classifier"]
    ...
    elif optim_type == "only_classifier":
        optimizer = AdamW(
            pl_module.fnd_classifier.parameters(), lr=lr, eps=1e-8, betas=(0.9, 0.98)
        )

    if pl_module.hparams.config["warmup_steps"] == 0:
        return optimizer
    ...
    warmup_steps = pl_module.hparams.config["warmup_steps"]  # 0.1 总步数的10%
    ...
    if decay_power == "cosine":
        ...
        scheduler = get_polynomial_decay_schedule_with_warmup(  # 热身部分学习率线性递增到设定值，由于power为1所以衰减部分是线性衰减到0
            optimizer,
