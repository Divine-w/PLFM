...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.transforms = keys_to_transforms(transform_keys, size=image_size)  # 图像数据增强
        ...
        if text_column_name != "" and not self.image_only:
            ...
            for i, texts in enumerate(self.all_texts):
                for _j in range(len(texts)):
                    self.index_mapper[j] = (i, _j)  # 如果一个图像只对应一个句子，则i == j, _j == 0，否则j为原始索引, i为图像索引, _j为每幅图对应的句子索引
                    j += 1
        else:
    ...
    def collate(self, batch):
        ...
        if len(txt_keys) != 0:
            for i, txt_key in enumerate(txt_keys):
                ...
                dict_batch[f"{txt_key}_ids"] = torch.tensor(input_ids)
                dict_batch[f"{txt_key}_masks"] = torch.tensor(attention_mask)
